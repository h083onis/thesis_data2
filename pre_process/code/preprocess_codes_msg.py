...
def process_codes(codes_list, codes_word_dict, type='train'):
    ...
    for code_dict in codes_list:
        ...
        if type == 'train':
            codes_word_dict = update_dict(token_filepath, codes_word_dict, kind='code')
        for kind in list(code_dict.keys())[1:]:
            for line in code_dict[kind]:
                ...
                if type == 'train':
                    codes_word_dict = update_dict(token_list, codes_word_dict, kind='code')
    return codes_list2, codes_word_dict


def process_msg(msg, msg_word_dict, type='test'):
    msg_list = tokenize_msg(msg)
    if type == 'test':
        msg_word_dict = update_dict(msg_list, msg_word_dict, kind='msg')
    return msg_list, msg_word_dict


def update_dict(word_list, word_dict, kind):
    if kind == 'msg':
